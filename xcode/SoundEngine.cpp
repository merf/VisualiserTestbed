...
void CSoundEngine::Init()
{
	...
	while (file = [dirEnum nextObject]) 
	{
		...
		{
			[mp_SaveFiles addObject:[file stringByDeletingPathExtension]];
		}
	}
	*/
	...
	if(randy)
	{		
		//std::string dir = getHomeDirectory() + "music/shunt mix";
	...
	else
	{
		...
		std::string path = dir + "music/test.mp3";
		mAudioSource = audio::load(path);
...
void CSoundEngine::Update()
{
	...
	if (mTrack->isPlaying() && mTrack->isPcmBuffering())
	{
		...
		if (mBuffer && mBuffer->getChannelData(CHANNEL_FRONT_LEFT))
		{
			...
			if (mSampleCount > 0 && mBuffer->getChannelData(CHANNEL_FRONT_LEFT)->mData != 0)
			{
				if(mSampleCount & 1)
				{
					mSampleCount--;
